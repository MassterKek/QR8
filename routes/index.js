...
router.get('/', async function(req, res, next) {
  ...
  axios.post('http://localhost:5000/scraper/v1/event/fetch_one', { q: 'coffee', loc: 'ma', orderBy: 'date_start' })
    .then((result) => {
...
router.post('/home_events', async function(req, res, next) {
  ...
  // probably need to call this when submit button is clicked
  axios.post('http://localhost:5000/scraper/v1/event/fetch_one', { q: `${q_lower}`, loc: 'ma', orderBy: 'covid_cases' })
    .then((result) => {
      // now result.data is an array of our events 
      if (result && result.data && result.data.length > 0) {
        res.render('pages/home', { title: 'QR8 - Home' , items: result.data});
